    ...
    render(){
        return(
            <div className={`jumbotron bg-cover text-center ${this.props.screenSize > 992 ? "background" : "background-small"}`}>
                ...
                <p className='mb-4'>
                    <Link to="/allgyms" className="lead mt-3 font-weight-bold"><u>Currently {this.props.totals.mats} open mats at {this.props.totals.gyms} gyms in {this.props.totals.states} cities</u> </Link>
                </p>
                ...
                <form className="form mt- my-2 my-lg-0 justify-content-center mx-auto" style={{maxWidth:'600px'}} value={this.state.searchTerm} onChange={e=>{this.setState({searchTerm:e.target.value})}} onSubmit={e=>{this.submitSearch(e)}}>
