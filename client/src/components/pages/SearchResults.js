...
class SearchResults extends React.Component {
    ...
    sortMats(){
        ...
        if(this.props.sortBy==='Distance'){
            let compare = (a,b) => {
                let comparison = 0
                let distanceA = ((lat - a.lat)*(lat - a.lat) + (lng - a.lng)*(lng - a.lng))
                let distanceB = ((lat - b.lat)*(lat - b.lat) + (lng - b.lng)*(lng - b.lng))
    ...
    renderSortBy(){
        ...
        return(
            <select style={{width:'50%'}} className='form-control ml-3' id='sort-by' value={this.props.sortBy} onChange={e=>this.props.setSortBy(e.target.value)}>
    ...
    renderResults(size){
        ...
        let matResults = this.sortMats().map(el=>{
            return(
                <CustomOpenMatCard size={size} key={el.id} mat={el} gymList={this.listGyms()}></CustomOpenMatCard>
            )
        })
    ...
    renderMediumResults(){
        return(
            <>
                <div className='container-fluid'>
                    <div>
                        <div className='h1 text-center my-4'>{this.props.openMats.length ? `Search Results (${this.props.openMats.length})` : 'Search Results'}</div>
                        {/* //this.renderGyms() returns a list of gyms from search results along with how many open mats were found at each one */}
                        {/* {this.renderGyms()} */}
    ...
    renderSmallResults(){
        return(
            <>
                <div className='container-fluid'>
                    <div>
                        <div className='h2 text-center my-4'>{this.props.openMats.length ? `Search Results (${this.props.openMats.length})` : 'Search Results'}</div>
                        {/* //this.renderGyms() returns a list of gyms from search results along with how many open mats were found at each one */}
                        {/* {this.renderGyms()} */}
                        <div className='d-flex align-items-center my-3 justify-content-center'>
                            Sort By: {this.renderSortBy()}
...
export default connect(mapStateToProps,{searchOpenmats, getGyms, refreshMap, setSortBy })(SearchResults)